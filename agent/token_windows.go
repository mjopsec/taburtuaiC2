...
// stealToken duplicates the primary token of pid.
...
func impersonateToken(pid uint32) (string, error) {
	...
	r, _, e := procImpersonateLoggedOnUser.Call(uintptr(imp))
	if r == 0 {
...
func makeToken(user, domain, password string) (windows.Token, error) {
	...
	if r == 0 {
		return 0, fmt.Errorf("LogonUser(%s\\%s): %v", domain, user, e)
	}
	return hToken, nil
}

// runAsToken spawns exe with args using the provided token.
func runAsToken(token windows.Token, exe, args string) error {
	...
	r, _, e := procCreateProcessWithToken.Call(
		uintptr(token),
		0,
